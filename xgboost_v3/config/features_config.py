...
FEATURE_GROUPS = {
    ...
    'market_features': [
        ...
        # УДАЛЕНО: hour_sin, hour_cos, dow_sin, dow_cos - временные признаки вызывают переобучение
    ],
...
EXCLUDE_COLUMNS = [
    # Целевые переменные и утечка данных
    'buy_expected_return', 'sell_expected_return',
    'buy_profit_target', 'sell_profit_target',  # КРИТИЧНО: целевые переменные!
    'buy_loss_target', 'sell_loss_target',      # КРИТИЧНО: целевые переменные!
    'buy_max_profit', 'sell_max_profit',        # КРИТИЧНО: информация из будущего!
    'buy_realized_profit', 'sell_realized_profit',  # КРИТИЧНО: информация из будущего!
    
    # Entry сигналы (могут содержать информацию из будущего)
    'is_long_entry', 'is_short_entry',
    'long_entry_confidence', 'short_entry_confidence',
    'long_entry_type', 'short_entry_type',
    
    # Базовые OHLCV (используем производные)
...
FEATURE_CONFIG = {
    ...
    # Параметры для скользящих статистик (уменьшено для избежания дубликатов)
    'rolling_windows': [20, 60],  # Только 2 окна вместо 4
