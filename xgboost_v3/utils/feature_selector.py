...
# Импортируем новый маппинг признаков
try:
    from config.feature_mapping import get_feature_category, get_category_targets, get_temporal_blacklist
    USE_FEATURE_MAPPING = True
except ImportError:
    USE_FEATURE_MAPPING = False
    logger.warning("⚠️ feature_mapping.py не найден, используем старую категоризацию")
...
class FeatureSelector:
    ...
    def _select_hierarchical(self, X: pd.DataFrame, y: pd.Series,
        ...
        logger.info("🏗️ Использую иерархический отбор признаков с распределением 85/2/10/3")
        
        # НОВОЕ: Применяем temporal blacklist сразу
        temporal_blacklist = get_temporal_blacklist()
        original_feature_count = len(feature_names)
        feature_names = [f for f in feature_names if f not in temporal_blacklist]
        removed_count = original_feature_count - len(feature_names)
        
        if removed_count > 0:
            logger.info(f"🚫 Исключено {removed_count} проблемных temporal признаков: {temporal_blacklist}")
        ...
        if USE_FEATURE_MAPPING:
            # Используем новый точный маппинг
            get_category = get_feature_category
        else:
            ...
            def get_category(feature):
                ...
                technical_patterns = [
                    ...
                    'bullish', 'bearish', 'inside_bar', 'pin_bar',
                    ...
                    '_ma_', '_std_', 'rolling', '_slope', '_change',
                    # Добавляем дополнительные технические признаки
                    'open_ratio', 'high_ratio', 'low_ratio', 'spread_approximation',
                    'higher_high', 'lower_low', 'volume_position', 'price_efficiency',
                    'volume_price_corr', 'position_in_', 'trend_1h', 'trend_4h',
                    # Бинарные технические признаки
                    'oversold', 'overbought', 'strong_trend', 'volume_spike', 'near_'
                ]
        ...
        if group_name:
            ...
            # ОБНОВЛЕНО: Минимизируем временные признаки и увеличиваем технические
            # Было: 85/2/10/3, стало: 85/2/8/5
            targets = get_category_targets() if USE_FEATURE_MAPPING else {
                'technical': 85, 'temporal': 2, 'btc_related': 8, 'symbol': 5
            }
            
            n_technical = int(self.top_k * targets['technical'] / 100)  # 85%
            n_temporal = max(0, min(1, int(self.top_k * targets['temporal'] / 100)))  # 2% но max 1
            n_btc = int(self.top_k * targets['btc_related'] / 100)     # 8%
            n_symbol = int(self.top_k * targets['symbol'] / 100)       # 5%
            n_other = self.top_k - n_technical - n_temporal - n_btc - n_symbol
            
            # ЖЕСТКОЕ ОГРАНИЧЕНИЕ: temporal не больше 1 признака для top_k <= 50
            if self.top_k <= 50 and n_temporal > 1:
                n_temporal = 1
                n_other = self.top_k - n_technical - n_temporal - n_btc - n_symbol
        
        selected = []
        
        # 1. Отбираем технические индикаторы (85%)
        ...
        if len(technical_features) < n_technical:
            ...
            # ИСПРАВЛЕНО: Добираем недостающие из других категорий (кроме temporal!)
            remaining_needed = n_technical - len(technical_features)
            if remaining_needed > 0:
                # Берем признаки из btc_related, symbol и other, но НЕ из temporal
                other_features = importance_df[
                    (~importance_df['feature'].isin(selected)) & 
                    (importance_df['category'].isin(['btc_related', 'symbol', 'other']))
                ]
                if len(other_features) > 0:
                    add_features = other_features.head(remaining_needed)['feature'].tolist()
                    selected.extend(add_features)
                    logger.info(f"   Добавлено {len(add_features)} признаков из других категорий для достижения квоты technical")
        else:
            selected.extend(technical_features.head(n_technical)['feature'].tolist())
        
        # 2. Отбираем временные признаки (2%) - минимум!
        ...
        # 4. Отбираем символы (3%)
        symbol_features = importance_df[importance_df['category'] == 'symbol']
        symbol_features = symbol_features[~symbol_features['feature'].isin(selected)]
        selected.extend(symbol_features.head(n_symbol)['feature'].tolist())
        
        # 5. Отбираем остальные признаки
        ...
        if len(selected) < self.top_k:
            ...
            # ИСПРАВЛЕНО: Приоритет отдаем НЕ временным признакам
            # Сначала добираем из btc_related и other
            non_temporal = all_remaining[all_remaining['category'] != 'temporal']
            if len(non_temporal) >= remaining:
                selected.extend(non_temporal.head(remaining)['feature'].tolist())
            else:
                # Если не хватает, добавляем все non-temporal и остаток из temporal
                selected.extend(non_temporal['feature'].tolist())
                still_needed = remaining - len(non_temporal)
                temporal_remaining = all_remaining[all_remaining['category'] == 'temporal']
                selected.extend(temporal_remaining.head(still_needed)['feature'].tolist())
        ...
        for cat in ['technical', 'temporal', 'btc_related', 'symbol', 'other']:
            count = final_counts.get(cat, 0)
            percentage = count/len(selected)*100 if selected else 0
            target = {'technical': 85, 'temporal': 2, 'btc_related': 10, 'symbol': 3, 'other': 0}.get(cat, 0)
            
            # Более жесткая проверка для temporal
            if cat == 'temporal':
                status = "✅" if percentage <= 3 else "❌ КРИТИЧНО!"
            else:
                status = "✅" if abs(percentage - target) < 10 else "⚠️"
                
            logger.info(f"   {cat}: {count} ({percentage:.1f}%) {status} [цель: {target}%]")
        
        # ЖЕСТКАЯ ПРОВЕРКА: если временных признаков слишком много, принудительно удаляем лишние
        temporal_count = final_counts.get('temporal', 0)
        max_temporal = max(1, int(self.top_k * 0.02))  # Максимум 2% или 1 признак
        if temporal_count > max_temporal:
            logger.warning(f"⚠️ Слишком много временных признаков: {temporal_count} > {max_temporal}")
            logger.warning("   Принудительно удаляем лишние временные признаки...")
            
            # Находим все временные признаки в отобранных
            temporal_in_selected = [f for f in selected if get_category(f) == 'temporal']
            
            # Сортируем их по важности (если есть scores)
            if self.feature_scores:
                temporal_in_selected.sort(key=lambda x: self.feature_scores.get(x, 0), reverse=True)
            
            # Оставляем только разрешенное количество
            temporal_to_keep = temporal_in_selected[:max_temporal]
            temporal_to_remove = temporal_in_selected[max_temporal:]
            
            # Удаляем лишние
            selected = [f for f in selected if f not in temporal_to_remove]
            
            logger.info(f"   Удалено {len(temporal_to_remove)} временных признаков")
            logger.info(f"   Оставлено временных признаков: {len(temporal_to_keep)}")
